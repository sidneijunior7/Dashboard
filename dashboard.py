...
if uploaded_file is not None:
    ...
    st.line_chart(df.set_index('DATE')['BALANCE'].diff().fillna(df['BALANCE'].iloc[1]))
    st.line_chart(df.set_index('DATE')['EQUITY'].diff().fillna(df['EQUITY'].iloc[1]))
    ...
    if start_date <= end_date:
        ...
        st.line_chart(filtered_df.set_index('DATE')['BALANCE'].diff().fillna(df['BALANCE'].iloc[1]))
        st.line_chart(filtered_df.set_index('DATE')['EQUITY'].diff().fillna(df['EQUITY'].iloc[1]))
    else:
